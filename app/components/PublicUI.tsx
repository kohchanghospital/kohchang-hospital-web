...
export function PublicHero({ title, eyebrow, description, image }: { title: string; eyebrow?: string; description?: string; image?: string }) {
  return (
    <header className="relative isolate overflow-hidden border-b border-[rgb(var(--color-border))] bg-[rgb(var(--color-secondary))]">
      ...
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(100deg,rgba(15,42,61,.98)_10%,rgba(15,76,92,.88)_58%,rgba(8,119,128,.68))]" />
      <div className="container-page py-14 sm:py-16 lg:py-20">
        <div className="max-w-3xl animate-soft-reveal">
          {eyebrow && <p className="mb-3 text-sm font-semibold uppercase tracking-[.12em] text-teal-200">{eyebrow}</p>}
...
export function HubCard({ href, title, description, icon }: { href: string; title: string; description?: string; icon?: React.ReactNode }) {
  return (
    <Link href={href} className="group surface-card flex min-h-40 flex-col p-5 transition duration-200 hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-[var(--shadow-md)] sm:p-6">
      <div className="flex items-start justify-between gap-4">
        {icon && <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-[rgb(var(--color-primary))]">{icon}</span>}
        <ArrowIcon />
      </div>
      <h2 className="mt-5 text-lg font-semibold leading-7 text-[rgb(var(--color-secondary))] group-hover:text-[rgb(var(--color-primary-dark))]">{title}</h2>
