...
function PageLink({ href, disabled, label, children }: { href: string; disabled: boolean; label: string; children: React.ReactNode }) {
  ...
  return <Link href={href} aria-label={label} className="flex h-10 min-w-10 items-center justify-center rounded-lg border border-[rgb(var(--color-border))] bg-white text-lg text-slate-600 hover:border-[rgb(var(--color-primary-border))] hover:bg-[rgb(var(--color-primary-light))]">{children}</Link>;
}
