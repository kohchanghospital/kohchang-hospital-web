...
export default async function VehicleDetailPage({ params }: { params: { lang: Lang; id: string } | Promise<{ lang: Lang; id: string }> }) {
    ...
    if (!schedule) return (
        <section className="container-page py-12 sm:py-16"><div className="surface-card mx-auto max-w-2xl p-8 text-center sm:p-12"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-teal-50 text-3xl text-teal-700"><i className="bi bi-car-front" /></div><h1 className="mt-5 text-2xl font-bold text-[rgb(var(--color-secondary))]">{lang === 'th' ? 'ไม่พบข้อมูลการใช้รถยนต์' : 'Vehicle schedule not found'}</h1><p className="mt-3 text-[rgb(var(--color-muted))]">{lang === 'th' ? 'รายการนี้อาจถูกลบหรือไม่มีอยู่ในระบบ' : 'This schedule may have been removed or does not exist.'}</p><Link href={`/${lang}/about/vehicle`} className="btn-primary mt-7">{backLabel}</Link></div></section>
    )
    ...
    return (
        <section className="container-page max-w-5xl py-10 sm:py-14">
            ...
            <article className="surface-card overflow-hidden shadow-[var(--shadow-md)]">
                <div className="border-b border-[rgb(var(--color-border))] bg-gradient-to-br from-teal-50 to-white p-6 sm:p-8">
                    <h2 className="text-2xl font-bold leading-snug text-[rgb(var(--color-secondary))] sm:text-3xl">{schedule.title}</h2>
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">{metadata.map(([label, value, icon]) => <div key={label} className="flex items-center gap-4 rounded-2xl border border-teal-100 bg-white/90 p-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-xl text-teal-700"><i className={`bi ${icon}`} /></span><div><p className="text-xs font-semibold uppercase tracking-wider text-[rgb(var(--color-muted))]">{label}</p><p className="font-semibold text-[rgb(var(--color-secondary))]">{value}</p></div></div>)}</div>
                </div>
                <div className="space-y-8 p-6 sm:p-8">
                    <section><h3 className="text-lg font-bold text-[rgb(var(--color-secondary))]">{lang === 'th' ? 'รายละเอียด' : 'Details'}</h3>{details.length ? <ol className="mt-4 space-y-3">{details.map((detail, index) => <li key={detail.id} className="flex gap-4 rounded-xl bg-[rgb(var(--color-background))] p-4"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-teal-700 text-sm font-bold text-white">{index + 1}</span><p className="pt-0.5 leading-relaxed">{detail.detail_text}</p></li>)}</ol> : <p className="mt-3 text-[rgb(var(--color-muted))]">{lang === 'th' ? 'ไม่มีรายละเอียดเพิ่มเติม' : 'No additional details'}</p>}</section>
