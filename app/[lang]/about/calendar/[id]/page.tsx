        ...
        return (
            <section className="container-page py-12 sm:py-16">
                <div className="surface-card mx-auto max-w-2xl p-8 text-center sm:p-12">
                    <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-teal-50 text-3xl text-teal-700"><i className="bi bi-calendar-x" /></div>
            ...
            <article className="surface-card overflow-hidden shadow-[var(--shadow-md)]">
                <div className="border-b border-[rgb(var(--color-border))] bg-gradient-to-br from-teal-50 to-white p-6 sm:p-8">
                    ...
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">
                        <div className="flex items-center gap-4 rounded-2xl border border-teal-100 bg-white/90 p-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-xl text-teal-700"><i className="bi bi-calendar3" /></span><div><p className="text-xs font-semibold uppercase tracking-wider text-[rgb(var(--color-muted))]">{lang === 'th' ? 'วันที่' : 'Date'}</p><p className="font-semibold text-[rgb(var(--color-secondary))]">{formatCalendarDate(activity.activity_date, lang)}</p></div></div>
                        <div className="flex items-center gap-4 rounded-2xl border border-teal-100 bg-white/90 p-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-xl text-teal-700"><i className="bi bi-clock" /></span><div><p className="text-xs font-semibold uppercase tracking-wider text-[rgb(var(--color-muted))]">{lang === 'th' ? 'เวลา' : 'Time'}</p><p className="font-semibold text-[rgb(var(--color-secondary))]">{formatCalendarTime(activity.start_time, activity.end_time, lang)}</p></div></div>
                    </div>
                </div>
